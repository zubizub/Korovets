...
if ($link['rel'] !== '') $a_rel = ' rel="' . trim($link['rel']) . '"';
...
if ($icon_image === '') $output_icon .=	'<i class="' . esc_attr(trim(implode(' ', $classes))) . '"></i>';
	...
	if ($media_code['type'] === 'image') $output_icon .= '<img'. (($position === 'right' || $position === 'left') ? ' style="max-width:none;"' : '').' src="' . $media_code['code'] .'" width="' . $media_code['width'] .'" height="' . $media_code['height'] .'" alt="'.$media_alt.'" />';
	else {
