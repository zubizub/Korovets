...
if ($link_to !== '') {
	...
	$a_rel = ($link['rel'] !== '') ? ' rel="' . trim($link['rel']) . '"' : '';
	if ($link['url'] !== '') $link_div = '<a class="col-link custom-link" href="'.esc_url($link['url']).'" target="'.($link['target'] !== '' ? $link['target'] : '_self').'" title="'.$link['title'].'"'.$a_rel.'></a>';
}
