...
$display_button = $button_content = $button_link = $link = $button_color = $size = $text_skin = $outline = $icon = $icon_position = $radius = $onclick = $rel = $btn_class = '';
...
if ( !$user_id ) //the user doesn't have posts
	return;
...
if ( $avatar !== 'hide' ) {
	...
	$avatar_output= '<div class="uncode-avatar-wrapper single-media uncode-single-media" style="width: ' . $actual_width . '">';
	$avatar_output.= '<div class="single-wrapper" style="max-width: ' . $actual_width . '">';
...
if ( $user_bio !== '' && $author_bio === 'yes' ) {
	...
	$content_out .= '<div class="author-profile-bio '.$sub_class.'">' . uncode_remove_wpautop($user_bio, true) . '</div>';
}
...
// Outlined class
if ($outline === 'yes') $btn_classes[] = 'btn-outline';
