...
if ( $attachment_ids ) {
	...
	<div class="thumbnails <?php echo 'columns-' . $columns; ?>"><?php
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			$image_resized = uncode_resize_image($image_attributes->id, $image_attributes->guid, $image_attributes->path, $image_metavalues['width'], $image_metavalues['height'], 2, ($crop ? 2 / $thumb_ratio : null), $crop);
			...
			echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<a href="%s" class="%s" title="%s" data-options="thumbnail: \''.$image_resized['url'].'\'" data-lbox="ilightbox_gallery-' . $gallery_id . '">%s</a>', $image_link, $image_class, $image_title, $image ), $attachment_id );
