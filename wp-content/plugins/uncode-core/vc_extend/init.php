...
function uncode_index_get_query()
{
	...
	if ( floatval( $matrix_amount ) != 0 ) {
		...
		for ($i = 0; $i < $matrix_amount; $i++) {
			...
			//Create matrix loop
			$html.= '<li class="list-list-item" data-id="' . $i . '">
						<div class="option-tree-setting">
							<div class="open"><span class="post-title">' . sprintf(__('Item no. %s', 'uncode'), '<span class="matrix-item-no">' . ($i + 1) . '</span>' ) . '</span><span class="grey size">width: <span class="single_width_factor"></span>, height: <span class="single_height_factor"></span></span></div>
